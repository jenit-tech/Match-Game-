...
class App extends Component {
  ...
  startTimer = () => {
    if (imagesList.length > 0) {
      this.intervalId = setInterval(this.onDecreaseSeconds, 1000)
    }
  }
  ...
  setActiveTabId = tabId => {
    this.setState({
      activeTabId: tabId,
      activeImageId: this.getRandomImageId(tabId),
    })
  }

  getNextTabId = () => {
    const {activeTabId} = this.state
    const currentTabIndex = tabsList.findIndex(tab => tab.tabId === activeTabId)
    const nextTabIndex = (currentTabIndex + 1) % tabsList.length
    return tabsList[nextTabIndex].tabId
  }

  setActiveImageId = id => {
    const {activeImageId} = this.state
    if (id === activeImageId) {
      const nextTabId = this.getNextTabId()
      this.setState(prevState => ({
        score: prevState.score + 1,
        activeTabId: nextTabId,
        activeImageId: this.getRandomImageId(nextTabId),
      }))
    } else {
  ...
  getRandomImageId = tabId => {
    ...
    return randomImage ? randomImage.id : ''
  }
  ...
  getActiveImagesUrl = () => {
    ...
    return activeImage ? activeImage.imageUrl : ''
  }

  resetGame = () => {
    const initialTabId = tabsList[0]?.tabId
    this.setState({
      activeTabId: initialTabId,
      activeImageId: this.getRandomImageId(initialTabId),
      seconds: 60,
      isStarted: true,
      score: 0,
      isGameOver: false,
  ...
  render() {
    const {activeTabId, seconds, score, isGameOver} = this.state
    const filteredImages = this.getActiveTabImage()
    const imageUrl = this.getActiveImagesUrl()
          ...
          {isGameOver ? (
            <div className="result-container">
              <img
                ...
                alt="trophy"
              />
              <p className="result-score">Your Score</p>
            ...
            <div className="card-container">
              <img src={imageUrl} className="active-image" alt="match" />
